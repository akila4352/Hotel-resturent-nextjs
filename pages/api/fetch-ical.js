export default async function handler(req, res) {
  ...
  // Helper: minimal valid iCal to return on error so callers (Booking.com) always get 200 + text/calendar
  const minimalICal = () =>
    [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
  ...
  try {
    ...
    if (!["http:", "https:"].includes(parsed.protocol)) {
      // If invalid protocol, return minimal iCal (200) — avoid JSON/html/error text that Booking.com would reject.
      console.error("fetch-ical: invalid protocol for url", url)
    ...
    if (!r.ok) {
      ...
      // IMPORTANT: return a valid iCal (200 + text/calendar) rather than JSON/error text to satisfy Booking.com
      res.setHeader("Content-Type", "text/calendar; charset=utf-8")
      res.status(200).send(minimalICal())
      return
    }
    ...
    if (!lc.includes("BEGIN:VCALENDAR") && !lc.includes("BEGIN:VFREEBUSY") && !lc.includes("BEGIN:VEVENT")) {
      ...
      // Return minimal valid iCal (200) instead of JSON so Booking.com receives an .ics response
      res.setHeader("Content-Type", "text/calendar; charset=utf-8")
      res.status(200).send(minimalICal())
      return
    }

    // Return plain iCal to client with the Booking.com-preferred MIME type
    res.setHeader("Content-Type", "text/calendar; charset=utf-8")
    res.status(200).send(text)
  } catch (err) {
    console.error("fetch-ical error:", err)
    // On unexpected server error, still return a valid iCal so Booking.com won't get JSON/HTML/error text
    res.setHeader("Content-Type", "text/calendar; charset=utf-8")
    res.status(200).send(minimalICal())
  }
}
